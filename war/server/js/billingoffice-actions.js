...
[
  ...
  {name: "Office buffer hours", inputId: "#billingoffices-officeBufferHours", formVal: "officeBufferHours", required: false}
];
...
uptempo.billingOffices.resetValidFields = function(validFields){
  ...
  validFields.push({name: "Office buffer hours", inputId: "#billingoffices-officeBufferHours", formVal: "officeBufferHours", required: false});
};
...
uptempo.billingOffices.tableFormatter = function(nRow, aData, iDisplayIndex) {
  ...
  var bannerDisplay = "";
  if (aData["bannerKey"]) {
    bannerDisplay = "<a class='office-banner-update'" +
                    " data-office-key='" + aData["key"] +
                    "' data-office-name='" + aData["officeName"] +
                    "' data-banner-key='" + aData["bannerKey"] +
                    "' data-banner-active='" + aData["bannerActive"] + 
                    "' data-banner-url='" + (aData["bannerLinkUrl"] || "") +
                    "' href='#'>Update Banner</a>";
  } else {
    bannerDisplay = "<a class='office-banner-update' data-office-key='" +
                    aData["key"] + "' data-office-name='" + aData["officeName"] +
                    "' href='#'>Add Banner</a>";
  }
  var bannerLink = $(bannerDisplay).click(uptempo.billingOffices.showBannerForm);
    ...
    $.ajax({
      ...
      success: function(response) {
          ...
          if (response.status == "SUCCESS") {   
            ...
            $("#billingoffices-officeBufferHours").val(response.data.officeBufferHours || '');
...
uptempo.billingOffices.clearBillingofficesForm = function() {
  ...
  $("#billingoffices-officeBufferHours").val("");
};
...
uptempo.billingOffices.showDeleteConfirm = function(billingofficesKey) {
  ...
  $("#billingoffices-confirm-popup-delete").on("click", uptempo.billingOffices.deleteOffice);

  //*** Show the form.
  $("#billingoffices-confirm-popup").popup("open");
};

uptempo.billingOffices.deleteOffice = function() {
...
uptempo.billingOffices.showBannerForm = function(e) {
  ...
  $("#office-banner-office-key").attr("value", officeKey);
  ...
  var bannerUrl = $(e.currentTarget).attr("data-banner-url");
...
uptempo.billingOffices.submitBannerForm = function(e, data) {
  ...
  var formData = "bannerKey=" + bannerKey +
                 "&bannerActive=" + toggleBanner +
                 "&bannerLinkUrl=" + bannerUrl; 
  //*** Submit the XHR request
    $.ajax({
      type: 'PUT',
